...
const SCHLUESSEL_MUSTER = [
  { name: 'Anthropic-Schlüssel', regex: /sk-ant-[A-Za-z0-9_-]{20,}/ },
  { name: 'OpenAI-Schlüssel', regex: /sk-(?!ant-)(proj-)?[A-Za-z0-9_-]{32,}/ },
  { name: 'Google-Schlüssel', regex: /AIza[0-9A-Za-z_-]{35}/ },
  { name: 'GitHub-Token', regex: /gh[pousr]_[A-Za-z0-9]{30,}|github_pat_[A-Za-z0-9_]{50,}/ },
  { name: 'privater Schlüssel', regex: /-----BEGIN [A-Z ]*PRIVATE KEY-----/ },
  { name: 'apiKey-Feld', regex: /["']api[_-]?key["']\s*[:=]\s*["'][^"']{20,}["']/i },
  { name: 'client_secret', regex: /["']client[_-]?secret["']\s*[:=]\s*["'][^"']{12,}["']/i },
]
...
export async function projektAnalysieren (id) {
  ...
  for (const d of dateien) {
    ...
    if (d.ext === '.html' || d.ext === '.htm') {
      ...
      seiten.push({ rel: d.rel, bytes: d.bytes, lang, titel })
