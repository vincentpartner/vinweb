...
export async function buildErzeugen (projekt) {
  ...
  const ersetzeInText = async (relDatei) => {
    ...
    for (const [alt, neu] of nameKarte) {
      ...
      // WICHTIG: auch URL-codierte Schreibweisen erfassen («Bewida%20Seite.dc.html»
      // in meta-refresh oder JS-Weiterleitungen) – sonst zeigen sie nach der
      // Umbenennung ins Leere (Bewida-Fund 12.09.).
      const schreibweisen = [...new Set([
        altName,
        encodeURIComponent(altName),
        altName.replace(/ /g, '%20'),
      ])]
      const muster = new RegExp('([="\'(/])(?:'
        + schreibweisen.map(v => v.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')
        + ')(["\'#?)])', 'g')
      const vorher = inhalt
...
async function schreibeBerichtUndRaus (id, ziel, bericht, urlKarte, nameKarte) {
  // Die .htaccess ist TECHNIK, kein SEO: Klartext-Adressen, Cache-Regeln,
  // Sicherheits-Kopfzeilen und die state.json-Freigabe braucht JEDER Build.
  // Läuft der SEO-Schritt (Profil mit Domain), schreibt er die volle Fassung
  // samt Weiterleitungen – fehlt er, liefern wir hier die Basis.
  // (Dynamischer Import, weil seo.js seinerseits aus build.js importiert.)
  try {
    await fs.access(path.join(ziel, '.htaccess'))
  } catch {
    try {
      const { htaccessBauen } = await import('./seo.js')
      await fs.writeFile(path.join(ziel, '.htaccess'),
        htaccessBauen({ site: {}, redirects: [] }), 'utf8')
      bericht.hinweise.push('Basis-.htaccess erzeugt (Klartext-Adressen, Cache, Sicherheit) – '
        + 'ohne SEO-Profil fehlen nur die Weiterleitungen alter Adressen.')
    } catch { /* lieber ohne .htaccess bauen als gar nicht */ }
  }
