...
export async function mechanischPruefen (ordner) {
  ...
  for (const rel of htmls) {
    ...
    for (const m of inhalt.matchAll(/(?:src|href|poster)="([^"]+)"/g)) {
      ...
      if (!vorhanden.has(voll) && !vorhanden.has(alsWurzel)) {
...
export async function endpruefungLaufen ({ projekt, anbieter, modell, onMeldung = () => {} }) {
  ...
  // Stand festhalten, auf den sich die Prüfung bezieht
  let stand = null
