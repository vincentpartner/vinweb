...
function seiteAuswerten (html, seitenUrl) {
  ...
  // 1. CSS-Reste wegputzen – Baukästen wie Tilda betten Stil-Schnipsel mitten
  //    in Inhalts-Elemente ein ("#rec123 .t-card{…} Eigentlicher Text").
  // 2. Leergeputzte Zeilen verwerfen.
  // 3. Exakte Wiederholungen (Navigation, doppelte mobile Menüs) nur 1× behalten.
  const gesehen = new Set()
  const text = zeilen
    .map(z => z.replace(/[#.@][^{}\s][^{}]*\{[^{}]*\}\s*/g, '').replace(/\s+/g, ' ').trim())
    .filter(z => z.length > 1)
    .filter(z => !/^[{}\s]*$/.test(z))
    .filter(z => {
      const kern = z.replace(/^[-#>\s]+/, '')
  ...
  // nav/footer zusätzlich: sonst landet die Navigation als Textzeilen im Ertrag.
...
export async function ernten ({ url, maxSeiten = 60, nurSeite = false, signal, onMeldung = () => {} }) {
  let start
  try {
  ...
  const basis = nurSeite ? slug(host + (start.pathname !== '/' ? '-' + start.pathname : '-startseite')) : slug(host)
  const ziel = path.join(ERNTE_DIR, `${basis}-${stempel}`)
  ...
  let warteschlange
  let perSitemap = false
  if (nurSeite) {
    // Nur genau diese eine Adresse – kein Crawling, keine Link-Verfolgung.
    onMeldung('Ernte nur diese eine Seite.')
    warteschlange = [start.href]
    maxSeiten = 1
  } else {
    onMeldung('Suche sitemap.xml …')
    warteschlange = await seitenAusSitemap(start.href, host, maxSeiten)
    perSitemap = !!warteschlange
    if (!warteschlange) {
  ...
  while (warteschlange.length && seiten.length < maxSeiten) {
    if (signal?.aborted) { gestoppt = true; onMeldung('Gestoppt – schliesse mit dem bisher Gesammelten ab.'); break }
  ...
  for (const [bildUrl, info] of alleBilder) {
    if (signal?.aborted) { gestoppt = true; break }
