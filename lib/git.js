...
import fsn from 'node:fs/promises'
import { GEHEIM_DATEINAME, geheimnisImInhalt } from './geheim.js'
...
export async function git (id, argumente, optionen = {}) {
  ...
  for (let versuch = 1; ; versuch++) {
    try {
      ...
      // Nie Authentifizierungswerte in Fehlermeldungen nach aussen reichen.
      if (e && typeof e.message === 'string') {
        e.message = e.message
          .replace(/Basic [A-Za-z0-9+/=]{8,}/g, 'Basic ***')
          .replace(/(GIT_CONFIG_VALUE_0=)[^\s'"]+/g, '$1***')
...
export async function sichern (id, nachricht) {
  ...
  const geaendert = zeilen.length
  if (!geaendert) return null

  // Review-Fund 6: Bevor irgendetwas committet (und automatisch zum
  // Fernlager gepusht) wird, werden die betroffenen Dateien auf Geheimnisse
  // geprüft. Funde fliegen aus dem Index, kommen in die .gitignore und
  // werden gemeldet – der Schlüsselwert selbst wird nie ausgegeben.
  const gewarnt = []
  for (const zeile of zeilen) {
    const rel = zeile.slice(3).replace(/^"|"$/g, '').split(' -> ').pop()
    if (!rel || zeile[0] === 'D') continue
    let verdacht = GEHEIM_DATEINAME.test(rel) ? 'Dateiname' : null
    if (!verdacht && /\.(js|json|php|txt|md|html?|css|xml|yml|yaml|env|ini|conf)$/i.test(rel)) {
      try {
        const st = await fsn.stat(path.join(quellPfad(id), rel))
        if (st.size < 1024 * 1024) {
          verdacht = geheimnisImInhalt(await fsn.readFile(path.join(quellPfad(id), rel), 'utf8'))
        }
      } catch { /* nicht lesbar – dann auch nicht committen? Lesbar für git wäre es trotzdem; konservativ weiter */ }
    }
    if (verdacht) {
      try { await git(id, ['rm', '-q', '--cached', '--ignore-unmatch', '--', rel]) } catch { /* dann bleibt sie eben unversioniert */ }
      try {
        const gi = path.join(quellPfad(id), '.gitignore')
        const bisher = await fsn.readFile(gi, 'utf8').catch(() => '')
        if (!bisher.split('\n').includes(rel)) {
          await fsn.writeFile(gi, bisher.replace(/\n*$/, '\n') + rel + '\n', 'utf8')
          await git(id, ['add', '--', '.gitignore'])
        }
      } catch {}
      gewarnt.push({ datei: rel, art: verdacht })
    }
  }

  // Nach dem Aussortieren nochmal zählen – vielleicht bleibt nichts übrig.
  const rest = (await git(id, ['status', '--porcelain'])).split('\n').filter(z => z.trim()).length
  if (!rest) return gewarnt.length ? { hash: null, dateien: 0, gewarnt } : null
  ...
  const ergebnis = { hash, dateien: geaendert }
  if (gewarnt.length) ergebnis.gewarnt = gewarnt
  return ergebnis
}
...
export async function zurueckSetzen (id, hash) {
  ...
  // 2./3. Review-Fund 4: KEIN pauschales "git clean" mehr – das konnte
  // Dateien löschen, die nur im ZIEL-Stand ignoriert waren (z. B. Geheimnisse).
  // Stattdessen wird exakt berechnet, welche heute versionierten Dateien es
  // im Zielstand nicht gab – nur DIESE werden entfernt. Ignoriertes und
  // Unversioniertes bleibt grundsätzlich unangetastet.
  const heute = (await git(id, ['ls-files'])).split('\n').filter(Boolean)
  const damals = new Set((await git(id, ['ls-tree', '-r', '--name-only', hash])).split('\n').filter(Boolean))
  try {
    await git(id, ['rm', '-r', '--cached', '-q', '.'])
    await git(id, ['checkout', hash, '--', '.'])
  } catch (e) {
    // Review-Fund 22: halb umgebauten Index nie zurücklassen.
    await git(id, ['reset', '-q', '--hard', 'HEAD'])
    throw new Error('Zurücksetzen fehlgeschlagen – Arbeitsstand wiederhergestellt. (' + e.message + ')')
  }
  for (const rel of heute) {
    if (!damals.has(rel)) {
      await fsn.rm(path.join(quellPfad(id), rel), { force: true })
    }
  }
  // Leer gewordene Ordner aufräumen (rein kosmetisch, Fehler egal).
  try { await git(id, ['clean', '-dq', '--', ...heute.filter(r => !damals.has(r)).map(r => r.split('/')[0]).filter((v, i, a) => v.includes('/') === false && a.indexOf(v) === i)]) } catch {}
...
// Review-Fund 21: Das Token wandert über die Prozess-UMGEBUNG zu Git,
// nicht über die Kommandozeile – so taucht es weder in der Prozessliste
// noch in Fehlermeldungen (die das volle Kommando enthalten) auf.
function authUmgebung (token) {
  if (!token) return {}
...
export async function fernAbgleichen (id) {
  ...
  await git(id, ['fetch', '-q', FERN, 'main'], { timeout: 30000, env: { ...process.env, ...auth } })
  // Auch nach festgefahrenen Kundenstaenden sehen - der Zweig existiert
  // nur, wenn die Kunden-Instanz einen Konflikt gemeldet hat.
  try {
    await git(id, ['fetch', '-q', FERN, 'kunde-wartet'], { timeout: 30000, env: { ...process.env, ...auth } })
  } catch {
...
export async function fernHochladen (id) {
  const auth = await fernAuth(id)
  try {
    await git(id, ['push', '-q', '-u', FERN, 'main'], { timeout: 120000, env: { ...process.env, ...auth } })
  } catch (e) {
...
export async function fernUebernehmen (id, strategie) {
  ...
  try {
    const auth = await fernAuth(id)
    if (stand.wartend > 0) {
      await git(id, ['push', '-q', FERN, '--delete', 'kunde-wartet'], { timeout: 30000, env: { ...process.env, ...auth } })
