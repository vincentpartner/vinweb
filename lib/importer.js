...
export async function zipImportieren (puffer, dateiname) {
  ...
  // Review-Fund 11: Grenzen gegen ZIP-Bomben – Anzahl, Einzelgrösse und
  // Gesamtgrösse werden anhand der ZIP-Kopfdaten VOR dem Entpacken geprüft.
  if (nutzbar.length > 25000) throw new Error('ZIP enthält über 25000 Dateien – das ist kein Website-Export.')
  let entpackt = 0
  for (const e of nutzbar) {
    const gr = e.header?.size || 0
    if (gr > 400 * 1024 * 1024) throw new Error('Eine Datei im ZIP wäre entpackt über 400 MB gross: ' + e.entryName)
    entpackt += gr
  }
  if (entpackt > 2 * 1024 * 1024 * 1024) throw new Error('ZIP wäre entpackt über 2 GB gross – abgebrochen.')
